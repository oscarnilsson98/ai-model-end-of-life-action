...
describe("GitHub input and workflow-command safety", () => {
  test("preserves hyphens in input environment names and trims values", () => {
    expect(inputEnvName("days-before-shutdown")).toBe("INPUT_DAYS-BEFORE-SHUTDOWN");
    expect(inputEnvName("models file")).toBe("INPUT_MODELS_FILE");
    expect(
      getInput("days-before-shutdown", {
        "INPUT_DAYS-BEFORE-SHUTDOWN": " 30 \n",
        INPUT_DAYS_BEFORE_SHUTDOWN: "wrong variable",
      }),
    ).toBe("30");
  });
