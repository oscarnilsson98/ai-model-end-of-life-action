...
A framework whose `semanticSupport` is `partial` reports the notice only for a file where none of its published rules resolved a model, so an understood file stays quiet while an unread selector shape is still named. A file whose tokenization failed reports no framework notice: its import list cannot be trusted, and `semantic-tokenization-incomplete@1` already states that the blob reached lexical fallback only.
