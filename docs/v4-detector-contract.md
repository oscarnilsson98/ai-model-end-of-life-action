# V4 Detector, Platform, and Feed Contract

Status: approved v4.0 detector boundary.

Two tiers resolve a model. A per-integration rule proves the serving platform and carries `high` confidence. The provider-agnostic model-selector rules resolve any static literal held by a key whose name selects a model, at `medium` confidence and without a proven platform, so a framework or SDK major this document does not name is still covered. Only text outside both tiers falls to the bounded lexical fallback.
...
The table is an override list for irregular labels, not an admission gate. Any other source-provider value derives its slug mechanically — case-folded, with each run of non-alphanumeric characters collapsed to a single hyphen — and every entry above is exactly what that derivation already produces. A derived slug is unregistered, which means no display name and no semantic platform proof; it does not mean nonblocking. Only a label from which no syntactically valid slug survives derivation has its rows skipped.

External evidence and trusted resolutions MUST use a registered slug to become platform-resolved or policy eligible, because a document supplied by a repository is asserting a platform rather than demonstrating one. Repository-detected evidence is not bound by that rule: an unregistered slug carries blocking authority once the evidence reaches the policy bar and the platform is established, either by the evidence or by the feed publishing the model ID for exactly one platform. A registry addition remains required for a display name, for semantic platform proof, and for the ambiguity tests that accompany a per-integration rule. Human-facing aliases may normalize to a slug, but canonical output never changes.
...
The per-integration matrix is intentionally bounded; the generic model-selector rules at the end of the table are not, and are what covers everything the matrix does not name. “Policy eligible” below is rule capability, not the final decision: blocking still requires `high` or `medium` confidence, `deployment` scope or trusted production environment, a resolved model, an established platform, an exact lifecycle join, and enabled enforcement.
...
| `source.ts.vercel-ai-sdk.openai-model@1`, `source.ts.vercel-ai-sdk.anthropic-model@1`, `source.ts.vercel-ai-sdk.google-model@1`, `source.ts.vercel-ai-sdk.google-vertex-model@1`, `source.ts.vercel-ai-sdk.azure-model@1`, `source.ts.vercel-ai-sdk.amazon-bedrock-model@1`, `source.ts.vercel-ai-sdk.cohere-model@1`, `source.ts.vercel-ai-sdk.groq-model@1`, `source.ts.vercel-ai-sdk.xai-model@1` | JavaScript/TypeScript | A provider instance or factory imported from `@ai-sdk/openai`, `@ai-sdk/anthropic`, `@ai-sdk/google`, `@ai-sdk/google-vertex`, `@ai-sdk/azure`, `@ai-sdk/amazon-bedrock`, `@ai-sdk/cohere`, `@ai-sdk/groq`, or `@ai-sdk/xai`, called directly or through a published model-factory member (`chat`, `responses`, `completion`, `languageModel`, `messages`, the embedding/image/video/speech/transcription/reranking members). The first positional argument is the selector | The provider package pins the platform: `openai`, `anthropic`, `google`, `google-vertex`, `cohere`, `groq`, `xai`, plus `azure` + `deployment-name` and `aws-bedrock` + `polymorphic`. A package serving one platform names that platform, so the registry is the only place a provider is enumerated. A `createX({ baseURL })` whose endpoint is custom, computed, or disagrees with the package stays platform-unresolved. Google resource, tuned, and publisher paths stay unresolved. `tools`, `files`, and `skills` are not model factories | OpenAI, Anthropic, and Google exact IDs yes; Azure and Bedrock only after trusted local resolution |
...
| `source.ts.generic.model-selector@1`, `source.py.generic.model-selector@1`, `config.hcl.generic.model-selector@1` | JavaScript/TypeScript, Python, HCL | Any static string literal, at any nesting depth, held by a key whose normalized name (lower-cased, `_` and `-` removed) is `model`, `modelid`, `modelname`, `embeddingmodel`, `engine`, `deployment`, `deploymentname`, or `azuredeployment`. No import, client, or framework is required, which is what makes this tier survive an SDK reshaping its call surface | The value MUST equal a feed model ID exactly, so the rule cannot introduce an identifier the feed does not publish. Platform is always `ambiguous`: nothing here inspects an SDK. `deployment`-family keys carry `deployment-name`, matching the Azure client and Terraform rules. Emits no unresolved-reference diagnostics, because a generic key with a dynamic value would fire in every repository holding a `model` key | Yes for a `model-id` selector outside `test`, `example`, `documentation`, and `unknown` scope, subject to the platform being established |
...
| npm | `@ai-sdk/google` | `4.0.31` |
| npm | `@ai-sdk/google-vertex` | `5.0.38` |
| npm | `@ai-sdk/azure` | `4.0.28` |
...
An unsupported integration is reported rather than left silent. When a tracked JavaScript, TypeScript, or Python file imports a recognized-but-unsupported LLM framework, the run publishes one `unsupported-integration-import.<frameworkId>@1` diagnostic per framework, naming the framework and a bounded sorted sample of the importing paths. The code carries the framework id because diagnostic aggregation groups by code and severity, so a shared code would collapse every framework into one entry; the framework registry bounds the number of possible codes. The diagnostic severity is `notice`: declared coverage stays `complete`, so enforcement never fails closed on an unsupported import alone. The diagnostic states what the degradation costs: a selection still held by a model-selector key is resolved generically at `medium` confidence without a proven platform, while anything outside that tier falls to lexical fallback, which cannot block, is named in notifications only as a low-confidence text match, and produces nothing at all when the selector is dynamic or the exact model ID is not literal-scan eligible.

A framework whose `semanticSupport` is `partial` suppresses the notice per import specifier, not per framework: an `@ai-sdk/openai` import is understood only when that provider's own rule produced a fact in the file, and a provider package outside the published set — `@ai-sdk/mistral`, say — is never understood, so one supported provider call cannot buy silence for an unsupported one beside it. The framework's own entrypoints (`ai`, `ai/rsc`) select no model on their own, so they are resolved against the whole snapshot rather than one file: `generateText({ model })` in a route handler stays quiet when the provider call it consumes resolved in another file, and is named when no provider call resolved anywhere. A file whose tokenization failed reports no framework notice: its import list cannot be trusted, and `semantic-tokenization-incomplete@1` already states that the blob reached lexical fallback only.

The recognized framework module prefixes are npm `ai` and `@ai-sdk` (Vercel AI SDK); npm `langchain` and `@langchain` plus Python `langchain`; npm `llamaindex` and `@llamaindex` plus Python `llama_index`; npm and Python `litellm`; npm `@google/generative-ai` and Python `google.generativeai`; and npm `@google-cloud/vertexai` plus Python `vertexai`. Each prefix is matched only in the ecosystem that publishes it, because the match forms differ: an npm prefix matches a bare specifier or a subpath, while a Python prefix also matches a dotted submodule (`llama_index.llms`) or an underscored sibling distribution (`langchain_openai`). Applying the Python forms to an npm-only prefix would claim every local `ai/` package and every `ai_*` module for the Vercel AI SDK. Type-only imports — the statement-level `import type { M } from "ai"` and the inline `import { type M } from "ai"` alike — bind nothing at runtime and produce no notice; an `export … from` re-export and a dynamic `import("…")` do reach the module, so both name a framework. Files whose tokenization already failed do not produce the notice; they already report incomplete semantic coverage.

Because the sampled paths are the only actionable part of the message, the sample is trimmed to the whole paths that fit inside the length the publisher renders, and the remainder is reported as a `(+N more)` count rather than cut mid-path.
...
The default v3 feed is a strict discriminated union. `PlatformSlug` is any syntactically valid lower-case slug; `CanonicalPlatformSlug` is the registered subset listed above. The distinction records what the detector knows about a platform, not whether its records may be enforced.
...
Every run validates the entire envelope before detector matching and publishes raw-source, normalized-feed, adapter-manifest, and active-record digests. The current public feed is untyped, so v3.0 includes a versioned adapter manifest that strictly parses every raw row and classifies the source's non-model rows by kind. The manifest carries no allowlist of reviewed source pairs: every well-formed row becomes a normalized record on the run that first sees it, so an upstream addition, rename or withdrawal is ordinary operation and does not by itself make scan coverage `partial`. Blocking authority depends on evidence strength and on the serving platform being established, not on the platform being registered.

Fields the adapter does not read are ignored rather than rejected, so an additive upstream column does not fail every consumer's run at once. Duplicate source pairs and malformed rows are quarantined individually with a bounded diagnostic and make coverage `partial`, keeping the first occurrence of a duplicated pair. Invalid manifest metadata and schema failures still produce `unknown + failed`. A provider label yielding no valid platform slug has its rows skipped with a bounded diagnostic and makes coverage `partial`; if no row resolves a platform, or no row is well formed at all, the non-empty feed contract also fails. A later release may replace the adapter with an approved typed endpoint.
