...
2. `advisory` when at least one actionable warning, conditional lifecycle risk, or review-overdue/stale/expired evidence source requires attention (or is new/worsened on a comparison run).
3. `no-actionable-risk` otherwise.

An unresolved selector never elevates the result by itself. A selector computed at runtime may be unresolvable by construction — a caller-supplied model override has no static value to find — so an elevation would pin the repository to a standing `advisory` that no repository change can clear, and the snapshot could name no finding to justify it. Unresolved selectors remain first-class output: they are counted in `unresolved`, published in `unresolved-references`, and listed in the job summary. Those whose model nothing could name are also stated once in a `selector-without-model-id` notice, described under [scan status](#scan-status); the Slack snapshot leaves them out.
...
A typed call site in application or deployment code whose model itself is unknown — computed at runtime, or an Azure deployment name or Bedrock selector that no trusted resolution mapped — likewise joins nothing. Such references are reported in one `selector-without-model-id` notice per run, with a reference count and a bounded path sample, under the same rules: a notice only, stated once in the summary, never sent to Slack, omitted during a feed outage, and reported for the target alone on comparisons. A reference that reached a finding was checked and is not listed, and one on a platform without feed records is named by `platform-without-lifecycle-data` instead, so no reference is named twice. A call that names an exact model through an unproven endpoint is not listed either: its literal is checked by the lexical fallback.
...
When a `platform-without-lifecycle-data` or `selector-without-model-id` notice is present, its message follows the evidence line as a `Not assessed:` line, so a clean result never reads as an all-clear for models nothing could check. Neither is repeated among the collapsed coverage diagnostics.
