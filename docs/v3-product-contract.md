...
Slack delivery in v3.0 is a stateless snapshot limited to current actionable lifecycle advisories, blocking findings, and configured evidence-source freshness state. Low-confidence lexical, documentation, test, and unchanged unresolved evidence is excluded by default. Delivery is attempted only when the event name is exactly `schedule`, `workflow_dispatch`, or `push` and the selected target is a commit. Every other event—including `pull_request`, `merge_group`, `release`, and local or unknown events—is skipped.
